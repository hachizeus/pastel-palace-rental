...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100 shadow-sm">
      ...
      {/* Mobile Menu */}
      <div className={cn(
        "fixed inset-0 top-16 bg-white z-40 p-4 transform transition-transform duration-300 md:hidden",
        isMenuOpen ? "translate-x-0" : "translate-x-full"
      )}>
        <div className="flex flex-col space-y-4">
          <Link to="/listings" className="p-4 rounded-2xl hover:bg-airbnb-light">
            Browse Stays
          </Link>
          <Link to="/become-a-host" className="p-4 rounded-2xl hover:bg-airbnb-light">
            Become a Host
          </Link>
          <Link to="/login" className="p-4 rounded-2xl hover:bg-airbnb-light">
            Login
          </Link>
          <Link to="/signup" className="p-4 rounded-2xl hover:bg-airbnb-light">
            Sign up
          </Link>
        </div>
      </div>
